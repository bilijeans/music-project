<template>
  <div class="station-detail">
    <div
      class="bg"
      :style="{
        backgroundImage: detailData ? `url(${detailData.columnPicUrl})` : '',
      }"
    ></div>
    <header>
      <i class="back wd-icon-thin-arrow-left" @click="backToStation"></i>
    </header>
    <main>
      <div class="radio-container">
        <div class="radio-cover">
          <img :src="detailData?.columnPicUrl" />
        </div>
        <div class="radio-info">
          <div class="radio-title">{{ detailData?.title }}</div>
      ...
      <div class="radio-desc">{{ detailData?.columnDes }}</div>
      <div class="radio-list-card">
        <div class="songs-play">
          <div class="play-all">
            ...
            <span>播放全部 {{ detailData?.list.length }}</span>
          </div>
        ...
        <div class="songs">
          <div class="songs-item" v-for="i in detailData?.list" :key="i.itemId">
            <!-- <span>{{ index + 1 }}</span> -->
            <div class="song-msg">
              <div class="songname">{{ i.title }}</div>
              <div class="song-info">
                <div class="publish-time">
                  {{ i.publishTime?.slice(0, 10) }}
                </div>
                <div class="duration">{{ dealWithTime(i.map?.mDuration) }}</div>
                <div class="play-num">{{ i.map?.songItem.playNumDesc }}</div>
              </div>
            </div>
            <svg
              @click="moreFunc(i.title)"
              class="icon"
              width="16px"
              height="16.00px"
              viewBox="0 0 1024 1024"
...
export default {
  ...
  created() {
    this.id = this.$route.query.radioId;
    this.getIndexData();
  },
  methods: {
    getIndexData() {
      this.$axios
        .get(
          `https://app.c.nf.migu.cn/MIGUM3.0/v1.0/template/radioMingXingDetail/release?columnId=${this.id}&templateVersion=7`
        )
        .then(({ data }) => {
          if (!data.data) {
            let str = data.slice(0, data.lastIndexOf("tags") - 2);
            str = str + "}}";
            let jsonData = JSON.parse(str);
            this.detailData = jsonData.data;
          } else {
...
.station-detail {
  ...
  main {
    ...
    .radio-desc {
      ...
      //     width: 100vw;
      //     height: 100%;
      //     background-image: linear-gradient(rgba(255, 255, 255, 0) 60%, #000);
      //   }
    }
    .radio-list-card {
      position: relative;
      ...
      //   box-shadow: 10px -50px 50px rgba(116, 116, 116, 0.2);
    }
  }
}
.radio-list-card {
