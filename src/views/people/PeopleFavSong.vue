<template>
  <div class="user-fav-song">
    <header>
      ...
      <div class="page-title">ta喜欢的音乐</div>
    </header>
    <main>
      <div class="play-head">
...
export default {
  ...
  computed: {
    getUserFavSongUrl() {
      return `/MIGUM3.0/resource/playlist/song/v2.0?pageNo=${this.pageNo}&pageSize=${this.pageSize}&playlistId=${this.id}`;
    },
  },
  created() {
    this.id = this.$route.query.id;
    this.getUserFavSongData();
  },
  methods: {
    goBack() {
      this.$router.go(-1);
    ...
    getUserFavSongData() {
      this.$axios.get(this.getUserFavSongUrl).then(({ data }) => {
